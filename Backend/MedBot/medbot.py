...
class Medbot:
    ...
    def list_cooccuring_symptoms(self, selected_symptoms):
        dis_list = set()
        counter_list = []

        for symp in selected_symptoms:
            dis_list.update(
                set(self.df_norm[self.df_norm[symp] == 1]['label_dis']))
        ...
        for dis in dis_list:
            ...
            row[0].pop(0)
            for idx, val in enumerate(row[0]):
                if val != 0 and self.DATASET_SYMPTOMS[idx] not in selected_symptoms:
    ...
    def get_cooccuring_symptoms(self, selected_symptoms, skip_symptoms):
        if len(selected_symptoms) <= 2:
            match_symp = self.list_cooccuring_symptoms(selected_symptoms)
        else:
